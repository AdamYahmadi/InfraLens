...
import { Globe, Send, Loader2, Cpu, HardDrive, Network, Clock, Sun, Moon, Layout, LayoutTemplate, Settings, PanelRightClose, PanelRightOpen } from 'lucide-react';
    ...
    <div className="py-3 border-b border-zinc-200 dark:border-white/5 last:border-0">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
          <Icon size={14} /><span className="text-xs font-medium">{label}</span>
        </div>
  ...
  return (
    <div className="flex h-screen w-screen bg-zinc-50 dark:bg-[#09090b] overflow-hidden text-zinc-800 dark:text-zinc-200 font-sans transition-colors duration-300">
      ...
      {loading && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-zinc-50/80 dark:bg-[#09090b]/80 backdrop-blur-sm transition-opacity duration-500">
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-white/10 p-8 rounded-2xl shadow-2xl flex flex-col items-center w-80">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 h-4 text-center">{loadingSteps[loadingStep]}</p>
            <div className="w-full h-1 bg-zinc-100 dark:bg-zinc-900 rounded-full mt-6 overflow-hidden">
              <div className="h-full bg-zinc-900 dark:bg-white transition-all duration-300 ease-out" style={{ width: `${loadProgress}%` }} />
            </div>
          </div>
        </div>
      )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedNode.data.sub_services.map((svc, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 px-3 py-1.5 rounded-lg text-xs font-mono">
                        <LayoutTemplate size={12} className="text-zinc-400" />{svc}
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <main className="relative flex-grow h-full bg-zinc-50 dark:bg-[#0d0d0d]">
        ...
        {/* Reopen button — only when chat is closed */}
        {!chatOpen && (
          <button
            onClick={() => setChatOpen(true)}
            title="Open AI chat"
            className="absolute top-4 right-4 z-30 flex items-center gap-2 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-white/10 text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100 text-xs font-medium px-3 py-2 rounded-xl shadow-lg transition-colors"
          >
            <PanelRightOpen size={15} /> AI Chat
          </button>
        )}
      </main>
      ...
      {chatOpen && (
        <aside className="w-[560px] bg-white dark:bg-zinc-950/50 flex flex-col border-l border-zinc-200 dark:border-white/5 shrink-0 z-20 backdrop-blur-xl">
          <div className="p-5 border-b border-zinc-200 dark:border-white/5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${isThinking ? 'bg-zinc-400 animate-pulse' : 'bg-emerald-500'}`} />
                <span className="text-[10px] font-bold text-zinc-400 uppercase">{isThinking ? 'Thinking' : 'Online'}</span>
              </div>
              <button
                onClick={() => setChatOpen(false)}
                title="Close chat"
                className="p-1.5 rounded-md hover:bg-zinc-100 dark:hover:bg-white/10 text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors"
              >
