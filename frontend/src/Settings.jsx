...
import { api } from "./api";
import { X, Loader2, Check, AlertTriangle, LogOut, ChevronDown, RefreshCw, Server, Terminal, Cpu } from "lucide-react";
  ...
  const detectModels = async (url, keepModel) => {
    ...
    try {
      ...
      setForm((f) => {
        ...
        if (found.length && !found.includes(want)) return { ...f, ollama_model: found[0] };
        return f;
      });
    } catch {
      setModels([]); setOllErr("The InfraLens backend isn't responding.");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-lg max-h-[86vh] bg-white dark:bg-[#0d0d0f] border border-zinc-200 dark:border-white/10 rounded-2xl shadow-2xl flex flex-col text-zinc-900 dark:text-zinc-100">
        ...
        <div className="px-6 h-14 border-b border-zinc-200 dark:border-white/5 flex items-center justify-between shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-zinc-100 dark:hover:bg-white/10 text-zinc-500 transition-colors"><X size={16} /></button>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-6 space-y-6">
              <Section icon={Server} title="Proxmox API" desc="Connection to your cluster">
                <Row>
                ...
                <Row>
                  <Field grow label="API user" value={form.pve_user} onChange={set("pve_user")} />
                  <Field grow label="Token name" value={form.pve_token_name} onChange={set("pve_token_name")} />
                </Row>
                <Field label={`Token value${tokenSet ? " · saved, leave blank to keep" : ""}`} type="password"
                  placeholder={tokenSet ? "••••••••••••" : "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"} value={form.pve_token_value} onChange={set("pve_token_value")} />
                <Toggle checked={form.pve_verify_ssl} onChange={set("pve_verify_ssl")}>Verify SSL certificate</Toggle>
              </Section>

              <Divider />

              <Section icon={Terminal} title="SSH probe" desc="Optional — detects services inside containers">
                <Row>
                  <Field grow label="SSH user" value={form.ssh_user} onChange={set("ssh_user")} />
                  <Field grow label={`SSH password${sshSet ? " · saved" : ""}`} type="password"
              ...
              <Section icon={Cpu} title="Local AI" desc="Ollama model used for chat">
                <label className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-400">Model</span>
                    <button type="button" onClick={() => detectModels()} disabled={detecting}
                      ...
                      <RefreshCw size={11} className={detecting ? "animate-spin" : ""} /> {detecting ? "Detecting…" : "Refresh"}
                    </button>
                  </div>
                  {models.length > 0 ? (
                    <div className="relative">
                      <select value={form.ollama_model} onChange={set("ollama_model")}
                        ...
                        {form.ollama_model && !models.includes(form.ollama_model) && (
                          <option value={form.ollama_model}>{form.ollama_model} (not installed)</option>
                        )}
                    ...
                    <div className="text-[12px] text-zinc-400 dark:text-zinc-500 rounded-lg border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-zinc-900/40 px-3.5 py-2.5">
                      {detecting ? "Detecting installed models…" : (ollErr || "No models detected. Click Refresh.")}
                    </div>
                  )}
                  ...
                  {showUrl && (
                    <div className="mt-1">
                      <Field label="Ollama URL" value={form.ollama_url} onChange={set("ollama_url")} />
                    </div>
                  )}
                </label>
              </Section>
              ...
              {result && (
                <div className={`flex items-start gap-2 text-[13px] rounded-lg px-3 py-2.5 border ${
                  ...
                  <span className={`mt-0.5 shrink-0 ${result.ok ? "text-emerald-500" : "text-rose-500"}`}>
                    {result.ok ? <Check size={14} /> : <AlertTriangle size={14} />}
                  </span>
                  <span className="text-zinc-600 dark:text-zinc-300 leading-relaxed">{result.msg}</span>
                </div>
              )}
            </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex flex-col">
          <h3 className="text-[13px] font-semibold leading-tight text-zinc-800 dark:text-zinc-100">{title}</h3>
          {desc && <span className="text-[11px] text-zinc-400 dark:text-zinc-500 leading-tight">{desc}</span>}
        </div>
      </div>
