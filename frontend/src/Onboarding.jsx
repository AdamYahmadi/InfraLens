...
export default function Onboarding({ onDone }) {
  ...
  const testProxmox = useCallback(async () => {
    ...
    try {
      ...
      setPveState(data.ok ? "ok" : "fail");
      setPveMsg(data.detail);
      if (data.ok) setTimeout(() => setStep(2), 400);
    } catch {
      setPveState("fail");
      setPveMsg("The InfraLens backend isn't responding.");
    } finally {
      setBusy(false);
    }
  }, [form]);
      ...
      setOllMsg(
        data.ok
          ? found.length
            ...
            : "Reachable, but no models are pulled. Run: ollama pull llama3"
          : data.detail,
      );
    } catch {
      setOllState("fail");
      setOllMsg("The InfraLens backend isn't responding.");
    } finally {
      setBusy(false);
    }
  }, [form]);
  ...
  const finish = useCallback(async () => {
    ...
    try {
      ...
      setOllState("fail");
      setOllMsg("Couldn't save. Is the backend running?");
    } finally {
      setBusy(false);
    }
  }, [form]);
    ...
    <div className="fixed inset-0 z-50 bg-zinc-50 dark:bg-[#09090b] text-zinc-900 dark:text-zinc-100 flex">
      ...
      <aside className="hidden md:flex w-[300px] shrink-0 flex-col justify-between border-r border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-[#0d0d0f] p-8">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="text-[11px] text-zinc-400 dark:text-zinc-600 leading-tight">
                      {s.desc}
                    </span>
                  </div>
                </div>
        ...
        <p className="text-[11px] text-zinc-400 dark:text-zinc-600 leading-relaxed">
          Everything stays on your network. Credentials are stored locally on
          this machine.
        </p>
      </aside>
          ...
          {step === 3 && (
            ...
            >
              {!hasModels ? (
                <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 px-4 py-6 text-center">
                  ...
                  <p className="text-[13px] text-zinc-500 dark:text-zinc-400 mb-4 max-w-[280px] mx-auto leading-relaxed">
                    Detect the models installed on your Ollama server, then pick
                    one.
                  </p>
                  <button
                    type="button"
              ...
              <Nav
                ...
                secondary={
                  ollState === "fail"
                    ? { label: "Finish without AI", onClick: finish }
                    : null
                }
              />
            </Stage>
  ...
  return (
    <div className="w-full max-w-md text-center flex flex-col items-center">
      ...
      <p className="text-[15px] text-zinc-500 dark:text-zinc-400 leading-relaxed mb-9 max-w-sm">
        ...
        container, and query your infrastructure in plain language — all on your
        LAN.
      </p>
      ...
      <button
        onClick={onBegin}
        className={btnPrimary + " w-full justify-center py-3 text-[15px]"}
      >
...
function Done({ onLaunch }) {
  return (
    <div className="w-full max-w-md text-center flex flex-col items-center">
      ...
      <p className="text-[15px] text-zinc-500 dark:text-zinc-400 leading-relaxed mb-9 max-w-sm">
        Your cluster is connected and InfraLens is ready to go. You can update
        any setting later from the gear icon in the sidebar.
      </p>
      <button
        onClick={onLaunch}
        className={btnPrimary + " w-full justify-center py-3 text-[15px]"}
...
function Status({ state, msg }) {
  ...
  const map = {
    testing: [
      "text-zinc-500 dark:text-zinc-400",
      <Loader2 key="i" size={14} className="animate-spin" />,
    ],
    ...
    <div className="flex items-center justify-between pt-3">
      {onBack ? (
        <button
          type="button"
          onClick={onBack}
          className="inline-flex items-center gap-1.5 text-[13px] text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
        >
      ...
      <div className="flex items-center gap-4">
        {secondary && (
          <button
            ...
            className="text-[13px] text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 transition-colors"
          >
            {secondary.label}
          </button>
        )}
        ...
        >
          ...
          {primary.label}
          {!primary.busy && <ArrowRight size={15} />}
        </button>
      </div>
    </div>
  );
