...
const SERVICE_COLORS = {
  dns: '#10b981', security: '#10b981', storage: '#0ea5e9', network: '#8b5cf6',
  dashboard: '#f43f5e', docker: '#06b6d4', database: '#f59e0b', host: '#71717a',
};
...
const getKind = (data) => {
  ...
  if (os.includes('PROXMOX') || os.includes('HOST') || tags.includes('host')) return 'HOST';
...
export default function ServiceNode({ data, selected }) {
  ...
  let tileClass, tileStyle = {}, iconStyle = {};
  if (selected || isHost) {
    tileClass = 'bg-zinc-900 dark:bg-zinc-100 border-zinc-900 dark:border-zinc-100 text-white dark:text-zinc-900';
  } else if (!isOnline) {
    tileClass = 'bg-zinc-50 dark:bg-zinc-900/50 border-zinc-200 dark:border-zinc-700/60 text-zinc-400 dark:text-zinc-600';
  } else {
    tileClass = '';
    tileStyle = { backgroundColor: `${color}1A`, borderColor: `${color}33` }; // ~10% bg, ~20% border
    iconStyle = { color };
  }
  ...
  return (
    ...
    >
      ...
      {isHost && <div className="absolute left-0 top-0 bottom-0 w-1 bg-zinc-900 dark:bg-zinc-100" />}

      <div className={`flex items-center gap-2.5 py-2.5 pr-3 ${isHost ? 'pl-4' : 'pl-3'}`}>
        <div className={`shrink-0 rounded-md p-1.5 border ${tileClass}`} style={tileStyle}>
          <Icon size={15} style={iconStyle} />
        </div>

        <div className="flex flex-col min-w-0 flex-1">
          <div className={`text-[13px] leading-tight truncate text-zinc-800 dark:text-zinc-100 ${isHost ? 'font-bold' : 'font-semibold'}`}>
            {data.label}
          </div>
